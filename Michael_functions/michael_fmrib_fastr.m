...
for c=1:m
    ...
    for sec=1:sections
        ...
        if isempty(intersect(exc,c)) & npc~=0
            ...
            [apc,ascore,asvar]=pca_calc(pcamat(1:(pccount-1),:)'); %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%55
            % [latent, latentLow,latentHigh]=pa_test_michael(pcamat(1:(pccount-1),:)')% to
            % many components remain
            %                 pa_test_michael(pcamat(1:(pccount-1),:)')
            ...
            if sec==1
                if ischar(npc)
                    ...
                    dddif=abs(diff(abs(diff(oev))));
                    dddif(end)=[];
                    TMPTH=find(arrayfun(@(x,y)x<y,dddif(4:end-1),dddif(5:end)))+3;
                    if ~isempty(TMPTH)
                        CUMVARTH_PC2=TMPTH(1);
                    else
                        CUMVARTH_PC2=0;
                    end
                    %                         pcs=round(mean([SLOPETH_PC CUMVARTH_PC2 length(dddif(dddif>std(oev(4:end-1)))) VAREXPTH_PC]));
                    %                         pcs=min(pcs,max([SLOPETH_PC CUMVARTH_PC2]));
                    %                          figure;nexttile;bar(oev);nexttile;plot(dddif);
                    pFit=abs(oev-polyval(polyfit([CUMVARTH_PC2+1:length(oev)-1]',oev(CUMVARTH_PC2+1:end-1),1),[1:length(oev)]'));
                    pFitLen=length(oev(pFit(1:end-1)>mean(pFit(1:end-1))*3));
                    %[SLOPETH_PC CUMVARTH_PC2 length(dddif(dddif>std(oev(4:end-1)))) VAREXPTH_PC]
                    pcs=min(pcs,pFitLen);
                    %                          altP=(abs(oev(1:end-1)-polyval(polyfit([CUMVARTH_PC2+1:length(oev)-1]',oev(CUMVARTH_PC2+1:end-1),1),[1:length(oev)-1]')))-mean(abs(oev(CUMVARTH_PC2+1:end-1)-polyval(polyfit([CUMVARTH_PC2+1:length(oev)-1]',oev(CUMVARTH_PC2+1:end-1),1),[CUMVARTH_PC2+1:length(oev)-1]')))*3;
                    %                          pcs=min(pcs,length(altP(altP>0)));
                    if paralellChan
                        %                           fprintf('\n%d residual PCs will be removed from channel %d\n',pcs,c);chanSubStr
                        if isnumeric(chanSub)
                            if length(chanSub)>1
                                fprintf('\n%d residual PCs will be removed from channel %d - subsection %d \n',pcs,chanSub(2), chanSub(1));
                            else
                                fprintf('\n%d residual PCs will be removed from channel %d - subsection %d \n',pcs,c,chanSub);
                            end
                        end
