...
%         EEG.etc.clean_sample_mask = sample_mask;
        EEG = eeg_addnewevents(EEG, {retain_data_intervals(:,1)'}, {'ASR_out'});
        EEG = eeg_addnewevents(EEG, {retain_data_intervals(:,2)'}, {'ASR_in'});
    else
        EEG = BUR;
    end
end
...
if ~strcmp(window_crit,'off') && ~strcmp(window_crit_tolerances,'off')
    ...
    EEG = clean_windows_michael(EEG,window_crit,window_crit_tolerances); 
end
