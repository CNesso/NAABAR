...
%         signal = eeg_addnewevents(signal, {retain_data_intervals(:,1)'}, {'ASR_out'});
%         signal = eeg_addnewevents(signal, {retain_data_intervals(:,2)'}, {'ASR_in'});

%         tt=zeros(size(retain_data_intervals,1)-1,69);
%         for i=2:size(retain_data_intervals,1)
%         tt(i,:)=[retain_data_intervals(i-1,2),retain_data_intervals(i,1),0.7,1,0.9 zeros(1,64)];
%         end
%         TMPREJ=unique([TMPREJ;tt],'rows');
globalASR=retain_data_intervals;
catch e
